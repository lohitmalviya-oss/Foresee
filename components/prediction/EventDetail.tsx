    ...
    >
      ...
      >
        ...
        <GlassCard className="p-12 relative border-white/10 shadow-[0_0_100px_rgba(99,102,241,0.1)] overflow-visible">
          ...
          <h2 className="text-4xl font-black text-white mb-8 leading-[1.1] tracking-tight line-clamp-3">{event.question}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
            <div className="p-8 bg-white/5 rounded-3xl border border-white/5">
              ...
              <div className="text-2xl font-black text-teal-400">{event.probability}% Yes / {100 - event.probability}% No</div>
            </div>
            <div className="p-8 bg-indigo-600/5 rounded-3xl border border-indigo-500/10 relative overflow-hidden">
              ...
              <div className="flex items-center gap-6">
                <div>
                  <div className="text-3xl font-black text-emerald-400">+{projectedGain}</div>
                  <div className="text-[8px] font-black text-slate-500 uppercase">Correct</div>
                </div>
                <div className="w-px h-8 bg-white/10" />
                <div>
                  <div className="text-3xl font-black text-rose-400">-{projectedLoss}</div>
          ...
          {isExpired && (
            <div className="mb-8 p-6 bg-slate-900 border border-slate-700 rounded-3xl text-center">
              ...
              <p className="text-slate-500 text-xs font-medium">Submission closed on {event.expiresAt.toLocaleDateString()}. Final correctness will determine Credibility Score impact.</p>
            </div>
          )}
          ...
          {!isAuthenticated ? (
            <div className="text-center p-12 bg-indigo-600/10 rounded-[2.5rem] border border-indigo-500/20">
              <h4 className="text-xl font-black text-white mb-4">Account Required</h4>
              <button onClick={onAuthClick} className="px-12 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-xl min-h-[44px]">Sign In to Participate</button>
            </div>
          ) : !hasVoted && !isExpired ? (
            <div className="space-y-12">
              <div className="px-4">
                <div className="flex justify-between items-center mb-6">
                  <span className="text-[11px] font-black text-slate-600 uppercase tracking-widest">Low Confidence</span>
                  <div className="flex flex-col items-center">
                    <span className="text-5xl font-black text-white uppercase tracking-tighter">{userPrediction}%</span>
                    <span className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em]">{userPrediction >= 50 ? 'FORECAST: YES' : 'FORECAST: NO'}</span>
                  </div>
                  <span className="text-[11px] font-black text-teal-600 uppercase tracking-widest">High Confidence</span>
                </div>
              ...
              <div className="flex flex-col gap-4">
                <motion.button 
                  ...
                  className={`w-full py-6 rounded-3xl font-black text-xl transition-all shadow-3xl min-h-[44px] ${isSubmitting ? 'bg-slate-800 text-slate-500 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-600/30'}`}
                >
            ...
            <div className="text-center py-10 bg-teal-500/10 rounded-[2.5rem] border border-teal-500/20">
              ...
              <h4 className="text-2xl font-black text-white mb-2 uppercase tracking-tighter">Forecast Synchronized</h4>
              <p className="text-teal-400 font-bold mb-6">Your analysis has been updated. Result expected to resolve after closing.</p>
            ...
            <div className="text-center py-10 bg-slate-800/50 rounded-[2.5rem] border border-white/5">
                <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Your Final Forecast</div>
                <div className="text-4xl font-black text-white mb-2">{existingVote.probability >= 50 ? 'YES' : 'NO'} ({existingVote.probability}%)</div>
